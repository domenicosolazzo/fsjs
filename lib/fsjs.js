(function(context){
    ...
    var removeDirectory = function removeDirectory(dirName, callback, recursively){
        ...
        var splitDirName = dirName.split("/");
        dirName = "/" + splitDirName.join('/');
    ...
    var duplicate = function duplicate( src, newName, callback ){
        var splittedName = src.split("/");
        src = "/" + splittedName.join("/");
        var srcEntry = fileEntries[src];
     ...
     * @param callback A callback function
     */
    var removeAll = function removeAll( callback  ){
        var reader = getFS().createReader();
        var response = packResponse({count:0}, [], {success:0});
        var entries = [];
        var num_removedFiles = 0;
        var internalDelete = function internalDelete( internalReader ){
            reader.readEntries( function( results ){
                if ( results.length ){
                    ...
                    if ( entries.length ){
                        num_removedFiles = entries.length;
                        entries.forEach( function( entry, i ){
                            if( entry.isFile){
                                entry.remove(function(){}, function(error){
                                    response.error.push( {code:-1, name:entry.name, error: error} );
                        ...
                        response.data.count = num_removedFiles;
                        response.metadata.success = 1;
                        callback.apply( outsideContext, [response]);
                    }
                ...
                response.error.push({code:-1, name: "ReaderError", error:error});
                callback.apply( outsideContext, [response]);
                return;
            });
        }
    ...
    var initialization = function initialization( ){
        ...
        storageType = {"PERSISTENT":1, "TEMPORARY":0}
    ...
    fsjs.isAvailable = config["CanIUseIt"];
